...
//     createModalItemRowWithDiscount(item, discountScope, ) {
...
//     // حساب الخصم على مستوى البند
//     const itemDiscountAmount = parseFloat(item.discount_amount || item.discount_amount || 0);
//     const itemDiscountValue = parseFloat(item.discount_value || item.discount_value || 0);
//     const itemDiscountType = item.discount_type || item.discount_type || 'amount';
//     const itemBeforeDiscount = parseFloat(item.total_before_discount || item.total_before_discount || originalTotal);
//     const itemAfterDiscount = parseFloat(item.total_after_discount || item.total_after_discount || (originalTotal - itemDiscountAmount));
...
//         console.log(discountScope,itemDiscountAmount);
//         console.log(
//          (discountScope === 'items' , itemDiscountAmount > 0) )

//     // التعديل: إضافة الأعمدة الجديدة فقط إذا كان الخصم على البنود
//     if (discountScope === 'items' && invoiceDiscountAmount > 0 ) {
//         const itemDiscountPercentage = itemDiscountType === 'percent' ? 
//             itemDiscountValue : ((itemDiscountAmount / itemBeforeDiscount) * 100);
        
//         itemHTML += `
//             <!-- قبل الخصم -->
//             <td>
//                 <span class=" ${ itemDiscountAmount > 0 ? 'text-decoration-line-through text-muted' : ''}">
...
//                 <div class="fw-bold">-${itemDiscountAmount.toFixed(2)} ج.م</div>
//                 <small class="text-muted">
...
//     }
//      else {
//         // إذا لم يكن هناك خصم على البند، نعرض العمود القديم فقط
//         itemHTML += `
//             <td   class="fw-bold">
...
createModalItemRowWithDiscount(item, discountScope, invoiceDiscountAmount) {
    ...
    if (discountScope === 'items' && invoiceDiscountAmount > 0 ) {
        ...
        const beforeDiscountClass = itemDiscountAmount > 0 ? 
            'text-decoration-line-through text-muted' : 
            'text-muted';
        
        const discountValueClass = itemDiscountAmount > 0 ? 
            'text-danger fw-bold' : 
            'text-muted';
        
        const discountPercentText = itemDiscountAmount > 0 ? 
            `(${itemDiscountPercentage.toFixed(1)}%)` : 
            '(0.0%)';
        ...
        itemHTML += `
            ...
            <!-- بعد الخصم -->
            <td class="${itemDiscountAmount > 0 ? 'text-success fw-bold' : 'fw-bold'}">
                ${itemAfterDiscount.toFixed(2)} ج.م
            </td>
        `;
    }
...
document.getElementById("printInvoiceItemsBtn")?.addEventListener("click", function () {
    ...
    const invoice = AppData.invoices?.find(inv => 
    ...
    totalElement.innerHTML = `
        <div class="amount-with-discount position-relative">
            ...
            <div class="discount-tooltip" 
                 ...
                 data-bs-title="الإجمالي: ${beforeDiscount.toFixed(2)} ج.م<br>
                                الخصم: -${discountAmount.toFixed(2)} ج.م<br>
                                نوع الخصم: ${discountScope === 'items' ? 'على البنود' : 'على الفاتورة'}">
                <!-- السعر الأصلي -->
                ...
                <!-- بادج الخصم -->
                <div class="discount-badge badge ${discountScope === 'items' ? 'bg-info' : 'bg-secondary'} position- top-10 end-10">
                    ${discountScope === 'items' ? '<i class="fas fa-tag me-1"></i>' : '<i class="fas fa-file-invoice me-1"></i>'}
                    ${discountType === 'percent' ? `${discountValue}%` : `${discountAmount.toFixed(2)} ج.م`}
                </div>
            </div>
        </div>
    `;
...
if (discountDetailsElement) {
    discountDetailsElement.innerHTML = `
        <div class="card ${discountScope === 'items' ? 'border-info' : 'border-secondary'} mb-3">
            <div class="card-header ${discountScope === 'items' ? 'bg-info' : 'bg-secondary'} text-white py-2 d-flex justify-content-between align-items-center">
                <div>
                    <i class="fas ${discountScope === 'items' ? 'fa-tag' : 'fa-file-invoice'} me-2"></i>
                    تفاصيل الخصم - ${discountScope === 'items' ? 'على البنود' : 'على الفاتورة'}
                </div>
                <span class="badge ${discountScope === 'items' ? 'bg-light text-info' : 'bg-light text-secondary'}">
                    ${discountType === 'percent' ? 'نسبة مئوية' : 'مبلغ ثابت'}
                </span>
            </div>
            <div class="card-body p-3">
                <div class="row">
                    ...
                    <div class="col-6">
                        ...
                        <div class="${discountScope === 'items' ? 'text-info' : 'text-secondary'} fw-bold">
...
const thead = document.querySelector("#invoiceItemsDetails").closest("table").querySelector("thead tr");
...
if (invoice.items?.length > 0) {
    ...
    invoice.items.forEach((item) => {
        const itemDiscountAmount = parseFloat(item.discount_amount || item.item_discount_amount || 0);
        const itemBeforeDiscount = parseFloat(item.total_before_discount || item.item_total_before_discount || 
                                           (item.quantity * (item.selling_price || item.price || 0)));
    ...
    // عرض البنود
    invoice.items.forEach((item) => {
        const row = this.createModalItemRowWithDiscount(item, discountScope, invoice.discount_amount || 0);
        tbody.appendChild(row);
    });
    ...
    if (discountScope === 'items' && discountAmount > 0) {
        const totalDiscountPercentage = totalBeforeDiscount > 0 ? 
            ((totalDiscountAmount / totalBeforeDiscount) * 100) : 0;
        ...
        totalRow.innerHTML = `
            <td colspan="${discountScope === 'items' ? '4' : '3'}" class="text-end">المبلغ الإجمالي:</td>
        ...
        totalRow.innerHTML = `
            <td colspan="${discountScope === 'items' ? '4' : '3'}" class="text-end">المبلغ الإجمالي:</td>
            <td colspan="${discountScope === 'items' ? '3' : '2'}" class="fw-bold">
                ${afterDiscount.toFixed(2)} ج.م
            </td>
        `;
    }
